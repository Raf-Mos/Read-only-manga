  ...
  useEffect(() => {
    const fetchNewestManga = async () => {
      try {
        ...
        setError(err?.message || 'Failed to fetch newest manga');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading newest manga...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
        <div className="flex items-center mb-4 sm:mb-0">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Newest Manga</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-1">Discover the latest manga releases</p>
          </div>
        </div>
